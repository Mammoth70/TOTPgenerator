# TOTP generator<br>Генератор TOTP

[![Android][1]][2] [![Kotlin][3]][4] [![GitHub license][5]][6] [![GitHub code size in bytes][7]]()

[1]: https://img.shields.io/badge/Android-9+-blue.svg?logo=Android&color=brightgreen
[2]: https://android.com/
[3]: https://img.shields.io/badge/Kotlin-2.2-blue.svg?logo=Kotlin
[4]: http://kotlinlang.org
[5]: https://img.shields.io/github/license/Mammoth70/TOTPgenerator.svg
[6]: LICENSE
[7]: https://img.shields.io/github/languages/code-size/Mammoth70/TOTPgenerator.svg?color=teal

Time-Based One-Time Password generator.  
Генератор одноразовых паролей на основе времени.  

Возможности приложения:
- добавление секретов генерации вручную или сканированием QR-кода (схема otpauth://);
- импорт секретов из Google Authenticator сканированием QR-кода (схема otpauth-migration://);
- поддерживаются алгоритмы HmacSHA1, HmacSHA256, HmacSHA512;
- поддерживается произвольный шаг времени;
- возможна генерация токенов TOTP длиной от шести до восьми символов;
- есть необязательный сдвиг времени генерации токенов вперёд до пяти секунд для улучшения UX;
- можно включить генерацию токена, следующего за текущим для авторизации на сервере с ушедшим вперёд временем;
- вход в приложение может быть защищён PIN-кодом или биометрией;
- секреты для генерации токенов и хеш PIN-кода хранятся в зашифрованном виде (AES256/GCM/NoPadding);
- сам ключ шифрования находится в защищённом хранилище AndroidKeyStore;
- можно нажатием скопировать текущий токен в буфер обмена;
- экспорт секретов в открытый или зашифрованный (AES256/GCM/NoPadding на ключе из хеша пароля PBKDF2WithHmacSHA256) файл JSON, импорт секретов из JSON;
- экспорт секретов в QR-код (схемы otpauth:// и otpauth-migration://);
- приложение не требует запроса разрешений, в том числе и разрешения "CAMERA permission";
- поддерживаются дневная, системная и ночная темы;
- поддерживаются английский и русский языки.  

## История
Подробности см. в файле [HISTORY.md](HISTORY.md).  

## Лицензирование
Данный проект распространяется по лицензии **GNU General Public License v3.0 (GPLv3)**  
Подробности см. в файле [LICENSE](LICENSE).  
Автор 2025-2026 Андрей Яковлев <andrey-yakovlev@yandex.ru>

Иконки Android Material Icons доступны по разрешительной лицензии Apache License 2.0,  
что означает, что их можно использовать бесплатно в личных, образовательных или коммерческих проектах.

## Licensing
This project is licensed under the **GNU General Public License v3.0 (GPLv3)**  
See the [LICENSE](LICENSE) file for details.  
Copyright 2025-2026 Andrey Yakovlev <andrey-yakovlev@yandex.ru>

Android Material Icons are available under the permissive Apache License 2.0,  
which means they are free to use for personal, educational, or commercial projects without cost.
