...
### 1.1.1
Чтение из потока теперь делается без вызова функции putData, код переписан через лямбду.  
...
### 1.5.0
Изменения во вводе, хранении и проверке PIN-кода.  
...
### 1.8.4
Переписан TokensAdapter. Удалён футер из кода, а в макет добавлены атрибуты "android:paddingBottom" и "android:clipToPadding", создающие и настраивающие внутренний отступ.

### 2.0.0
Переименованы файлы и классы:
- файл Parser переименован в OTPauthUriParser;
- файл Secret переименован в SecurityCrypto;
- файл SecretPin переименован в SecurityPin;
...
Внесены соответствующие изменения в функции файлов OTPAuthUriParser и SecurityPin.  
...
- вызовы извне этих функций теперь обёрнуты в lifecycleScope.launch;
...
### 2.6.5
Изменения в файле OTPauthUriParser.  
...
Объект TokensRepository переименован в TokensTrigger и вынесен в отдельный файл.  
Функция checkBiometricInDevice вынесена в отдельный файл.

### 2.6.7
Незначительные изменения в тестах.

### 2.6.8
Изменения в объекте OTPauthDataRepo:
- добавлена функция initialize асинхронно вызывающая readAllSecrets;
- в Application вместо readAllSecrets вызывается initialize;
- удалён флаг isDatabaseReady;
- вместо строки databaseError используется интерфейс MutableSharedFlow;
- соответственно переделана функция clearDatabaseError;
- теперь MainActivity подписывается на databaseError через collect, вместо вызова delay(100).

### 2.7.0
Появилась возможность экспорта и импорта секретов в файл JSON в открытом или зашифрованном виде.
- добавлены ExportActivity и класс OTPauthAdapter для выбора выгружаемых секретов;
- добавлены PasswordExportDialog и PasswordImportDialog для ввода пароля шифрования расшифрования;
- добавлен LoadingDialog для ожидания расшифрования;
- добавлены функции secretsToJson, secretsFromJson, encryptAndWrite и readAndDecrypt;
- добавлены необходимые вызовы в ExportActivity и MainActivity;
- внесены соответствующие изменения в файлы ресурсов;
- добавлен интеграционный тест связки функций secretsToJson и secretsFromJson.

### 2.8.0
Появилась возможность экспорта секретов в QR-код по схеме "otpauth-migration://".
- обновлены объявления зависимостей в файле libs.versions.toml;
- внесены соответствующие изменения в файл build.gradle.kts проекта;
- добавлен QrExportDialog для показа QR-кода;
- добавлены функции generateMigrationUri и generateQrCode;
- добавлены необходимые вызовы в ExportActivity и класс OTPauthAdapter;
- внесены соответствующие изменения в файлы ресурсов;
...
### 2.9.1
Несовместимые со схемой "otpauth-migration://" секреты больше не выгружаются через неё.
Несовместимыми считаются секреты для генерации семиразрядных токенов, или с периодом генерации не равным 30-ти сек.
- внесены соответствующие изменения в функцию startDialogExportQR и класс OTPauthAdapter;
- внесены соответствующие изменения в файлы ресурсов.

### 2.9.2
Исправлен цвет чекбокса в ExportActivity для ночной темы.

### 2.9.3
В список несовместимых со схемой "otpauth-migration://" добавлены секреты, длина которых не кратна восьми символам Base32.
...
### 2.9.6
Исправлена ошибка, из-за которой после загрузки приложения мог не отображаться список токенов.
В функцию initialize добавлен вызов триггера обновления sendCommandUpdate.

### 2.10.0
Теперь уникальность имени секрета задаётся полями issuer:label, а не полем label, как раньше.
Внесены соответствующие изменения в класс OTPauthDataBase, в файл OTPauthEntity и в объект OTPauthDataRepo.

### 2.10.1
В классе OTPauthDialog к полю ввода label добавлен фильтр, не позволяющий ввести двоеточие.

### 2.10.2
Теперь дополнительно поддерживается Андроид 9. MinSdk = 28. 

### 2.10.3
Добавлено описание деревьев макетов.

### 2.10.4
Изменения в файле SecutityCrypto.
- удалён вызов устаревшего SecureRandom.getInstance(SHA1PRNG) и подмешивание в него системного времени;
- удалён вызов setUnlockedDeviceRequired(true), в результате обеспечена совместимость с некоторыми китайскими устройствами;
- все вызовы криптографических функций обёрнуты в блоки "try catch";
- исправлена опечатка в теге при вызове функции LogSmart.
...
### 2.10.11
Добавлено форматирование вывода токенов. Разделение дефисом на две группы цифр.

### 2.10.12
Смена версии Gradle на 9.6.1.  
Переход на AGP 9.3.1.  
Обновлены версии зависимостей.

### 2.10.13
Изменены ресурсы ic_launcher. Изображение отзеркалено по вертикальной оси.